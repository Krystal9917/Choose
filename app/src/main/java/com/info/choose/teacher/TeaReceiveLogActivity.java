...
public class TeaReceiveLogActivity extends ListActivity {
    ListView listView;
    ArrayList listItems;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    json.put("id", teacher_id);
                    // http request defined in the root path
                    String response = HttpUtils.request("/receiveLogs", json);
                    JSONObject result = new JSONObject(response);
                    JSONArray logs = (JSONArray) result.get("logs");
                    for (int i = 0; i < logs.length(); i++) {
                        JSONObject log = logs.getJSONObject(i);
                        Map<String, Object> results = new HashMap<>();
                        results.put("name", log.get("student_name"));
                        results.put("major", log.get("student_major"));
        ...
        handler = new Handler(Looper.myLooper()) {
            ...
            public void handleMessage(Message message) {
                if (listItems.size() != 0) {
                    adapter = new SimpleAdapter(TeaReceiveLogActivity.this,
                            listItems,
                            R.layout.receive_list_item,
                            new String[]{"name", "major", "topic", "status", "request_extra", "response_extra"},
                            new int[]{R.id.name, R.id.major, R.id.topic, R.id.status, R.id.request_extra, R.id.response_extra});
